...
func Register(client WhatsUpClient, user string) (context.Context, error) {
	...
	// Lets first create a base context
	ctx := context.Background()

	// Lets then grab the authentication token
	token, ok := client.Connect(ctx, &Registration{SourceUser: user})

	if token == nil || ok != nil {
		return nil, errors.New("Error when trying to create a token")
	}

	// Lets then add the token to the context
	ctx_with_token := metadata.NewOutgoingContext(ctx, metadata.Pairs("token", token.Token))

	return ctx_with_token, nil
}
